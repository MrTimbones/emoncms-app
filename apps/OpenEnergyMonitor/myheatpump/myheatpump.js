...
config.app = {
    ...
    "heatpump_wcT":{"type":"feed", "autoname":"heatpump_wcT", "optional":true, "description":"Weather Compensation Temperature"},
    ...
    if (item) {
        ...
        if (previousPoint != item.datapoint) {
            ...
            if (viewmode=="powergraph")
            {
                ...
                else if (item.series.label=="WCT") { name = "Weather Comp"; unit = "°C"; dp = 1; }
...
function powergraph_load() 
{
    ...
    if (feeds["heatpump_wcT"]!=undefined) {
        data["heatpump_wcT"] = feed.getdata(feeds["heatpump_wcT"].id,view.start,view.end,view.interval,0,0,skipmissing,limitinterval);

        if (all_same_interval) {
            powergraph_series.push({label:"WCT", data:remove_null_values(data["heatpump_wcT"]), yaxis:2, color:"#ccc"});
        } else {
            powergraph_series.push({label:"WCT", data:data["heatpump_wcT"], yaxis:2, color:"#ccc"});
        }
    }
    ...
    if (data["heatpump_wcT"]!=undefined) feedstats["heatpump_wcT"] = stats(data["heatpump_wcT"]);
    ...
    var names = {
      ...
      "heatpump_wcT":"Weather comp temperature",
...
function powergraph_draw() 
{
    ...
    var options = {
        ...
        selection: { mode: "x" },
        legend:{position:"NW", noColumns:11}
    }
